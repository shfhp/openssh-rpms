...
LIST_RPMDIR(){
    DISTVER=$(GUESS_DIST)
    RPMDIR=$__dir/$(GUESS_DIST)/RPMS/$(rpm --eval '%{_arch}')
...
GET_OS() {
	...
	if [ "${dist}" == "el7" ]; then
		dist=$(rpm --eval '%{?dist}')
		if [ -z "${dist}" ]; then
			dist=.$(rpm -q glibc | rev | cut -d. -f2 | rev)
		fi
		echo "${dist}"
	else
		echo ."${dist}"	
...
if [[ ! -d $rpmtopdir ]]; then 
  echo "This script works only in el5/el6/el7/amzn1/amzn2/amzn2023/GETRPM/RPMDIR/GETEL/GETOS"
  echo "eg: ${0} el7"
  exit 1
fi
